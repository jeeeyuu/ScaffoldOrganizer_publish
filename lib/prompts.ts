...
3. 큰 목표를 실행 가능한 next action으로 쪼갭니다. 예: "자료 검토"는 상황에 따라 "검토할 자료 목록 만들기", "우선순위 높은 자료부터 읽기", "핵심 결정사항 메모하기"처럼 분해합니다.
   ...
   예: "custom prompt가 worklog와 command router에 반영되는지 확인하고, gemini api가 brain dump 처리 잘하는지 확인하고, UI 불편한 거 없는지 확인"은 "custom prompt 반영 경로 점검하기", "Gemini brain dump 처리 품질 테스트하기", "UI 사용 중 불편한 지점 정리하기"처럼 나눕니다.
...
priority 기준:
- 1: 오늘 바로 막고 있는 오류, 마감, 배포/저장 실패, 사용자가 당장 검증해야 하는 핵심 리스크
- 2: 기능 품질/정확도 확인, 사용자 이탈 가능성이 있는 UX 문제, API 처리 품질 검증
- 3: 일반 구현/정리/문서화/테스트 확장
