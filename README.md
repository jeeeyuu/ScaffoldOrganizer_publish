...
- `lib/prompts.ts`: `making/api_prompt.md`에서 정리된 AI 역할을 코드 레지스트리로 옮긴 파일
...
- tab 구조: `Inbox / Active / Long-term / Sessions / Work Log / Done`
- 메인 패널 안의 item grid
- session split view
- worklog split view
...
- 아이템 조회/수정/삭제
- 세션 저장/불러오기
- 브레인덤프 로컬 구조화 fallback
- 업무일지 draft 생성/저장
- 하단 커맨드 입력 기반의 기본 command routing
- Supabase 미설정 시 메모리 fallback

## 아직 남아 있는 작업

- 실제 OpenAI API adapter 연결
- `command_router / classifier / task_structurer / worklog_writer`를 프롬프트 레지스트리와 직접 연결
- Telegram polling worker를 Next 서버와 분리된 background process로 설계
- 인증/권한 모델 및 RLS 정책 설계
...
2. Supabase 프로젝트 연결
3. OpenAI server adapter 추가
4. Telegram ingestion worker 설계
