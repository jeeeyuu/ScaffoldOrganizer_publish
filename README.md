...
Scaffold Organizer는 자연어 brain dump를 실행 가능한 할일로 정리하고, 일정/캘린더/worklog까지 한 화면에서 관리하는 개인 생산성 앱입니다. 기존 바닐라 프로토타입을 `Node.js + React + Next.js + Supabase` 구조로 옮긴 프로젝트입니다.
...
- `Gemini API`: brain dump 분해, item classification
- `Demo store`: 개발 환경에서 로그인 없이 GUI를 확인하기 위한 메모리 fallback

## 주요 기능

- Supabase Auth 기반 회원가입/로그인, 회원가입 시 optional nickname 입력
- 로그인/회원가입 화면 상단 안내 문구는 `content/auth-intro.json`에서 관리
- 로그인 전 실제 앱 차단, 개발 preview에서는 demo user로 UI 확인
- 36시간 게스트 체험, 로그인/회원가입 시 게스트 데이터 자동 병합
- 이메일 인증 완료 안내 페이지: `/auth/confirmed`
- Settings에서 별도 계정 삭제 페이지로 이동, 삭제 후 자동 로그아웃
...
- Admin variables preset catalog: `content/admin-variable-presets.json`
- Settings에서 계정 정보, nickname, personal prompt, calendar week start, app info, user guide 접근

## AI 처리 구조

하단 입력창은 brain dump 전용 입력입니다.
...
- Gemini 미설정 시 줄바꿈/문장 단위 fallback으로 inbox item을 생성합니다. Gemini가 설정된 상태에서 429 등 API 실패가 발생하면 조용히 fallback하지 않고 오류를 표시합니다.
- 상태 변경, 일정 추가, Worklog 생성은 UI 버튼과 각 탭의 전용 기능으로 처리합니다.
...
스키마가 변경되는 기능을 추가한 경우, 앱 코드 배포와 별도로 Supabase Dashboard의 SQL Editor에 `supabase/schema.sql` 내용을 수동 적용해야 합니다.
...
- `user_settings.nickname`은 회원가입 optional nickname 또는 Settings에서 수정한 이름을 저장합니다.
- `admin_variables`는 관리자 화면에서 저장하는 운영 변수입니다. 입력 preset은 `content/admin-variable-presets.json`에서 관리합니다.
- 게스트 체험 테이블은 service role 전용으로 접근합니다.
...
게스트 체험, admin variables, 계정 삭제는 서버에서 `SUPABASE_SERVICE_ROLE_KEY`를 사용합니다.

회원가입 시 optional nickname은 Supabase Auth `user_metadata.nickname`에 먼저 저장되고, 로그인 후 `user_settings.nickname` 초기값으로 동기화됩니다.
...
이메일 인증 메일의 redirect는 `/auth/confirmed`로 연결됩니다. 이 페이지는 앱 내부 진입용이 아니라 인증 완료 안내용입니다.
...
![Login screen](./public/screenshot/login.svg)

### Active

![Active tab](./public/screenshot/active.svg)

### Calendar

![Calendar page](./public/screenshot/calendar.svg)

### Settings

![Settings page](./public/screenshot/settings.svg)
...
- 36시간 게스트 체험 데이터는 별도 guest 테이블에 저장되며, 계정 로그인/회원가입 시 사용자 DB로 병합됩니다.
- 배포 전 검증은 `npm run build`를 기준으로 합니다.
