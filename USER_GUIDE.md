...
Scaffold Organizer는 머릿속에 흩어진 할일, 일정, 현재 상태를 한 번에 적어두고 정리하는 개인용 작업 정리 도구입니다.
...
- `nickname`: 선택 입력입니다. 입력하면 Settings의 계정 정보와 nickname 초기값으로 사용됩니다.

이메일 인증이 필요한 경우, Supabase 인증 메일을 누르면 인증 완료 안내 페이지가 열립니다. 안내 페이지가 보이면 해당 창은 닫아도 됩니다.

로그인 없이 먼저 써보고 싶다면 로그인 화면의 `로그인 없이 체험하기` 버튼을 사용할 수 있습니다.

- 게스트 체험은 36시간 동안 유지됩니다.
- 36시간 안에 로그인하거나 회원가입하면 게스트 데이터가 계정 DB로 병합됩니다.
- 시간이 지나면 게스트 데이터는 삭제 대상이 됩니다.

개발 환경에서 `DEV_PREVIEW_AUTH_BYPASS=true`이면 로그인 없이 demo user 화면을 볼 수 있습니다. 이 demo 데이터는 UI 확인용입니다.

## 2. 기본 화면 구조

상단에는 앱 이름과 주요 버튼이 있습니다. 닉네임과 이메일은 메인 헤더에 항상 표시되지 않고 Settings 화면 상단의 계정 카드에서만 확인합니다.

- `Download`: 현재 item, schedule, worklog를 Markdown으로 다운로드합니다.
- `Settings`: 개인 설정, 보관 목록, 앱 정보, 사용법 페이지로 이동합니다.
- `Admin`: 관리자 계정에서만 보입니다.
- `Save to account`: 게스트 체험 중일 때만 보이며, 게스트 데이터를 계정에 저장하기 위한 로그인/회원가입 화면을 엽니다.
- `Logout`: 로그아웃합니다.
...
- Settings의 `Delete Account` 버튼을 누르면 별도 계정 삭제 페이지로 이동합니다.
- 안내를 확인한 뒤 `DELETE`를 입력해야 삭제가 진행됩니다.
- 계정이 삭제되면 현재 로그인 세션도 자동으로 로그아웃됩니다.
